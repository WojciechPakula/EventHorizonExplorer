...
public class TestRayMethod3 : MonoBehaviour
{
    ...
    float getBlackHoleAngle(Mass bc, Ray ray)//zrobic!!!! ignoruje promien
    {
    ...
    bool calcRay3(Ray _ray)
    {
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            if (!state)
            {
                addPoint(_ray.origin + v1 + v2 + v3 * 300);
                break;
            }
    ...
    void solveFullState(Ray ray, int bhInd, out Vector3 rot, out Vector3 v1, out Vector3 v2, out Vector3 v3, out float angle, out Vector3 axis)
    {
        ...
        angle = getBlackHoleAngle(_BlackHoles[bhInd], ray);
        axis = normalize(getAxis(ray, _BlackHoles[bhInd].transform.position));
        float d = getApproachCollisionDistance(ray, _BlackHoles[bhInd].transform.position);
        v1 = d * ray.direction;
        Vector3 hitBack = v1 + ray.origin;
        Vector3 hitFront = rotatePointAroundPoint(hitBack, _BlackHoles[bhInd].transform.position, axis, angle);
        v2 = hitFront - hitBack;
    ...
    bool solveCrossVector(Ray ray, Vector3 rot, Vector3 v1, Vector3 v2, Vector3 v3, int[] forbidden, int forbiddenC, out Ray _ray2, out Vector3 _rot, out Vector3 _v1, out Vector3 _v2, out Vector3 _v3, out int choosenBH)
    {
        ...
        for (int i = 0; i < (int)c; ++i)
        {
            if (contains(forbidden, forbiddenC, i))
                continue;
            Vector3 nextBlackHole = _BlackHoles[i].transform.position;
        ...
        if (isCross)
        {
            ...
            Vector3 axis2 = normalize(Vector3.Cross(choosenDir, _BlackHoles[choosenBH].transform.position - choosenPos));
            _rot = axis2 * beta2;

            _v1 = (choosenPos - _ray2.origin);
            _v2 = rotatePointAroundPoint(choosenPos, _BlackHoles[choosenBH].transform.position, axis2, beta2) - choosenPos;
            _v3 = normalize(rodrigez(choosenDir, axis2, beta));
        }
        return isCross;
    }
    bool crossCondition(Vector3 startPos, Vector3 axis, float beta, Vector3 center, int bhId)
    {
        Vector3 nextBlackHole = _BlackHoles[bhId].transform.position;
    ...
    void startPosition(Ray ray, out Vector3 v1, out Vector3 v2, out Vector3 v3, out int[] forbidden, out int forbiddenC, out bool shadowHit)
    {
        ...
        for (int i = 0; i < c; ++i)
        {
            if (contains(forbidden, forbiddenC, (int)i))
                continue;
            var center = _BlackHoles[i];
            Vector3 axis = (getAxis(ray, center.transform.position));
            float d = getApproachCollisionDistance(ray, center.transform.position);
            if (d <= 0)
            {
                //za lub posrednio
                ...
                {
                    //bh shadow
                    v2 = center.transform.position - ray.origin;
                    shadowHit = true;
                    return;
                }
                beta = beta % (2 * PI);
                Vector3 vecBack = (ray.origin + ray.direction * d) - center.transform.position;
                Vector3 dir = ray.origin - center.transform.position;
        ...
        for (int i2 = 0; i2 < c; ++i2)
        {
            if (contains(forbidden, forbiddenC, (int)i2))
                continue;
            var center = _BlackHoles[i2];
            Vector3 axis = normalize(getAxis(ray, center.transform.position));
            float d = getApproachCollisionDistance(ray, center.transform.position);
            if (d > 0)
            {
            ...
            else
            {
                ...
                if (angleAdvanced(vecBack, dir, axis) <= beta)
                {
                    ...
                    if (beta >= 90)
                    {
                        //bh shadow
                        v1 = ray.direction * d;
                        v2 = center.transform.position - ray.origin - v1;
                        shadowHit = true;
                        return;
                    }
                    ...
                    for (uint _i = 0; _i < 10; ++_i)
                    {
                        ...
                        int bhInd;
                        bool state = solveCrossVector2(ray, rot, mv1, mv2, mv3, center.transform.position, forbidden, forbiddenC, out ray2, out orot, out ov1, out ov2, out ov3, out bhInd);
                        ...
                        if (state)
                        {
                            if (length(orot) >= 90)
                            {
                                //bh shadow
                                v1 = ray.direction * d;
                                v2 = _BlackHoles[bhInd].transform.position - ray.origin - v1;
                                shadowHit = true;
                                return;
                            }
                            v2out += ov2;
    ...
    void solveMiddleState(Ray ray, int bhInd, out Vector3 rot, out Vector3 v1, out Vector3 v2, out Vector3 v3)
    {
        ...
        Vector3 axis = normalize(getAxis(ray, _BlackHoles[bhInd].transform.position));
        Vector3 rod1 = rodrigez(ray.direction, axis, angle / 2.0f);
        float d = getApproachCollisionDistance(ray, _BlackHoles[bhInd].transform.position);

        Vector3 hitBack = planeIntersection(_BlackHoles[bhInd].transform.position, ray.direction, ray.origin, rod1);
        Vector3 hitFront = rotatePointAroundPoint(hitBack, _BlackHoles[bhInd].transform.position, axis, angle);
    ...
    bool nextPosition(Ray ray, float rayLength, int[] forbiddenIn, int forbiddenC, out Vector3 v1, out Vector3 v2, out Vector3 v3, out int[] outForbidden, out int outForbiddenC, out bool shadowHit)
    {
        ...
        if (frontId >= 0)
        {
            ...
            float beta;
            Vector3 axis;
            solveFullState(extendedRay, frontId, out rot, out mv1, out mv2, out mv3, out beta, out axis);
            if (length(rot) >= 90)
            {
                //bh shadow
                v2 = _BlackHoles[frontId].transform.position - (ray.origin + v1);
                shadowHit = true;
                return false;
            }
            v2out += mv2;
            ...
            for (int _i = 0; _i < 10; ++_i)
            {
                ...
                Vector3 ov1, ov2, ov3, orot;
                int bhInd;
                bool state = solveCrossVector2(extendedRay, rot, mv1, mv2, mv3, _BlackHoles[frontId].transform.position, forbidden, forbiddenC, out ray2, out orot, out ov1, out ov2, out ov3, out bhInd);
    ...
    //bool crossCondition
    Vector3 getCrossPoint(Vector3 startPos, Vector3 endPos, Vector3 axis, Vector3 center, Vector3 bh)
    {
        Vector3 diff = endPos - startPos;
        Vector3 normal = bh - center;
        normal = rodrigez(normal, axis, PI/2.0f);
        return planeIntersection(bh, normal, startPos, diff);
    }
    bool nextPosition2(Ray ray, float rayLength, int[] forbiddenIn, int forbiddenC, out Vector3 v1, out Vector3 v2, out Vector3 v3, out int[] outForbidden, out int outForbiddenC, out bool shadowHit)
    {
        v1 = new Vector3(0, 0, 0);
        v2 = new Vector3(0, 0, 0);
        v3 = new Vector3(0, 0, 0);
        ...
        if (frontId >= 0)
        {
            ...
            {
                //bh shadow
                v2 = _BlackHoles[frontId].transform.position - (ray.origin + mv1);
                shadowHit = true;
                return false;
            }

            Vector3 startPos = ray.direction + mv1;
            Vector3 endPos = startPos + mv2;
            Vector3 center = _BlackHoles[frontId].transform.position;
            RotVector += rot;
            for (int i = 0; i < c; ++i)
            {
                if (contains(forbidden, forbiddenC, i))
                    continue;
                Vector3 bh = _BlackHoles[i].transform.position;
                bool isCross = crossCondition(startPos, axis, beta, _BlackHoles[frontId].transform.position, i);
                if (isCross)
                {
                    Vector3 p = getCrossPoint(startPos, endPos,normalize(Vector3.Cross(startPos, endPos)), center, bh);

                    Vector3 diff = endPos - startPos;
                    Vector3 miniVector = p - startPos;


                    if (length(normalize(diff) - normalize(miniVector)) < 0.01f)
                    {
                        //Front

                        float newDistance = length(miniVector);
                        float gamma = angleAdvanced(startPos - center, bh - center, axis);
                        Ray tmpExtendedRay = CreateRay(p, rodrigez(ray.direction, axis, gamma));

                        Vector3 _rot, _mv1, _mv2, _mv3, _axis;
                        float _beta;
                        solveFullState(tmpExtendedRay, i, out _rot, out _mv1, out _mv2, out _mv3, out _beta, out _axis);

                        if (length(_rot) >= 90)
                        {
                            //bh shadow
                            v2 = _BlackHoles[i].transform.position - (ray.origin + v1);
                            shadowHit = true;
                            return false;
                        }
                        forbidden[forbiddenC] = i;
                        forbiddenC++;
                        outForbidden[outForbiddenC] = i;
                        outForbiddenC++;
